...
### 每个 system 的 paragraph 强制要求（这次升级 — 必须更细致）：
- **字数：150-250 字**（不可少于 150 字 — 销售员要讲 1 分钟以上）
- **必须引用至少 3 个用户的具体回答**（用 \`**粗体**\` 包起来）
- **必须解释生理机制 + 数据机制**（为什么这些症状/数值合起来是危险的，引用具体医学逻辑）
- **必须连接到「拖延时间」**（如果用户答了 duration 不是「还没注意过」）
- **如果有体检数值**，必须引用至少 1 个数值 + 跟正常范围对比

✅ 250 字范例（高风险肾脏）：
「你刚才说 **尿泡 3 分钟还散不掉**，加上 **夜尿 3 次以上**、**长期吃止痛药 5 个月**，这三个组合在国际肾脏指南（KDIGO 2024）里被列为肾小球过滤膜开始受损的早期典型信号。机制上：肾小球本来不该让蛋白漏出，尿里看到密集泡沫代表过滤膜出现「漏洞」；夜尿增加代表肾浓缩功能下降，肾在『提早交班』；止痛药（NSAIDs）会直接收缩肾血管，长期等于每天逼肾加班。再配合化验：你的 **肌酐 135 µmol/L** 超出男性上限（62-106），eGFR 已降至 G3a，意味着滤网剩余功能约 45-60%。这是「沉默到不沉默」的临界点。」
